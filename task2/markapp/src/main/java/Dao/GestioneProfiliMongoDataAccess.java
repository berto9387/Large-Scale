...
public class GestioneProfiliMongoDataAccess extends MongoDataAccess{
    ...
    public static int trovaUtenteInBaseAlRuolo(Utente utente,String squadra,String nazione,String ruolo){
        Document doc=null;
        try{
            doc = collectionSocieta.aggregate(Arrays.asList(match(and(eq("nomeSocieta", squadra), eq("nazione", nazione))), lookup("utenti", ruolo.toLowerCase(), "_id", "utente"), project(include("nomeSocieta", "nazione", "utente._id", "utente.nome", "utente.cognome", "utente.email","utente.ruolo")))).first();
            //1)
        } catch(Exception e){
            return 2;
        }
